...
;(function () {
  try {
    ...
    electron.contextBridge.exposeInMainWorld("zcodeModelHub", {
      ...
      readState: function () {
        return electron.ipcRenderer.invoke("modelhub:read-state");
      },
      writeState: function (st) {
        return electron.ipcRenderer.invoke("modelhub:write-state", st);
      },
      readTemplates: function () {
        return electron.ipcRenderer.invoke("modelhub:read-templates");
      },
    });
  } catch (e) {
    try {
